...
var RLANG = {
	...
	mailto: 'Email',
	web: 'URL',
	video_html_code: 'แปะลิงค์ Youtube เช่น http://www.youtube.com/watch?v=sHPAsR9PGzQ',
	file: 'Insert File...',	
	upload: 'Upload',
	download: 'Download',
	choose: 'Choose',
	or_choose: 'Or choose',
	drop_file_here: 'สามารถนำไฟล์มาวางในกรอบนี้ ขนาดไฟล์สูงสุด 300KB (JPG,GIF,PNG)',
...
(function($){
	...
	Redactor.prototype = {
		...
		resizeImage: function(resize)
		{
			...
			$(resize).hover(function() { $(resize).css('cursor', 'pointer'); }, function() { $(resize).css('cursor','pointer'); clicked=false; });
		...
		insertVideo: function()
		{
			...
			//console.log(data);
			var code = '';
			code = data.split(/http[s]*:\/\/www\.youtube\.com\/watch\?v=([a-zA-Z0-9_-]+)/i);
			if(code[1]==undefined){
				code = data.split(/http[s]*:\/\/www.youtube.com\/embed\/([a-zA-Z0-9_-]+)/i)[1];
			}else{
				code =code[1];
			}
			data = "[youtube]"+code+"[/youtube]";
			if ($.browser.msie)
			{
		...
		imageSave: function(el)
		{
			...
			if (link !== '')
			{
				if ($(parent).get(0).tagName !== 'A')
				{
					$(el).replaceWith('<a href="' + link + '" rel="nofollow">' + this.outerHTML(el) + '</a>');
				}
				else
				{
			...
			var handler = $.proxy(function()
			{
				...
				if (this.opts.imageGetJson !== false)
				{
					$.getJSON(this.opts.imageGetJson, $.proxy(function(data) {
						...
						{
							var img = $('<img style="max-height: 500px;max-width:650px; display:inline-block;" src="' + val.thumb + '" />');
		...
		{
			this._imageSet('<img style="max-height: 500px;max-width:650px; display:inline-block;" alt="" src="' + $(e.target).attr('rel') + '" />', true);
		},
		imageUploadCallbackLink: function()
		{
			if ($('#redactor_file_link').val() !== '')
			{
				var data = '<img style="max-height: 500px;max-width:650px; display:inline-block;" src="' + $('#redactor_file_link').val() + '" />';
		...
		_imageSet: function(json, link)
		{
			...
			{
				data = $.parseJSON(json);		
				html = '<p><img style="max-height: 500px;max-width:650px; display:inline-block;" src="' + data.filelink + '" /></p>';
			}
			else
			{
				html = json;
		...
		insertLink: function()
		{
			...
			this._insertLink('<a href="' + link + '" rel="nofollow" target="_blank">' +  text + '</a> ', $.trim(text), link);
		...
		fileUploadCallback: function(json)
		{
			...
			var link = '<a href="' + data.filelink + '" rel="nofollow" target="_blank">' + text + '</a>';
				...
				if (n.nodeType === 3) 
				{
					...
					if (html)
					{
						html = html.replace(/&/g, '&amp;')
									.replace(/</g, '&lt;')
									.replace(/>/g, '&gt;')
									.replace(url1, '$1<a href="http://$2" rel="nofollow" target="_blank">$2</a>$3')
									.replace(url2, '$1<a href="$2" rel="nofollow" target="_blank">$2</a>$5');
